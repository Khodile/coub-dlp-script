...
CONFIG_FILE="$HOME/.config/coub-dlp"

# Create config file with default variables
if [[ ! -e "$CONFIG_FILE" ]]
then
  echo "MINIMAL_DURATION_SECONDS=45" > "$CONFIG_FILE"
  echo "FREE_SPACE_RESERVE_KiB=102400" >> "$CONFIG_FILE"
  echo 'YT_DLP_ARCHIVE_FILE="$HOME/.coub-dlp/yt-dlp/downloaded.txt"' >> "$CONFIG_FILE"
  echo 'YT_DLP_BATCH_FILE="$HOME/.coub-dlp/yt-dlp/batch-file.txt"' >> "$CONFIG_FILE"
  echo 'LOGS_DIRECTORY="$HOME/.coub-dlp/logs/"' >> "$CONFIG_FILE"
  echo 'YT_DLP_TARGET_DIRECTORY="$HOME/.coub-dlp/coubs/temp/"' >> "$CONFIG_FILE"
  echo "YT_DLP_OUTPUT_TEMPLATE='%(title)s by <%(uploader)s> [%(id)s].%(ext)s'" >> "$CONFIG_FILE"
  echo 'FINAL_TARGET_DIRECTORY="$HOME/.coub-dlp/coubs/final/"' >> "$CONFIG_FILE"
fi
